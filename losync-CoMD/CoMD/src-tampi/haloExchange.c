...
#pragma oss task shared(ll) \
    in(ll->secondFreeSentinel[0]) \
    out(ll->firstMallocSentinel[0])
    {
        // Allocate Memory
        // Separate set of send buffers for each box
        ll->sendBufs = comdMalloc(ll->numColumns*sizeof(char**));
        // Separate set of send buffers for each box
        ll->recvBufs = comdMalloc(ll->numColumns*sizeof(char**));
        // Separate set of send requests for each box
        ll->sendRequests = comdMalloc(ll->numColumns*sizeof(MPI_Request*));
        // Separate set of receive requests for each box
        ll->recvRequests = comdMalloc(ll->numColumns*sizeof(MPI_Request*));
        // Number of bytes to be sent for each box
        ll->nSends = comdMalloc(ll->numColumns*sizeof(int*));
        // Number of bytes to be received from each neighbour
        ll->nRecvs = comdMalloc(ll->numColumns*sizeof(int*));
        // Need to store receive statuses so that message info can be accessed
        // outside the current task
        ll->recvStatus = comdMalloc(ll->numColumns*sizeof(MPI_Status*)); 
    }
...
#pragma oss task firstprivate(iColumn) \
        shared(ll) \
        in(ll->firstMallocSentinel[0]) \
        out(ll->secondMallocSentinel[iColumn])
        {
            // Different request for each neighbour
            ll->sendRequests[iColumn] = comdMalloc(ll->columnNumNeighbours[iColumn]*sizeof(MPI_Request));
            // Seperate number of bytes received from each neighbour
            ll->nSends[iColumn] = comdMalloc(ll->columnNumNeighbours[iColumn]*sizeof(int));
            ll->nRecvs[iColumn] = comdMalloc(ll->columnNumNeighbours[iColumn]*sizeof(int));
            // Receive statuses for each neighbours
            ll->recvStatus[iColumn] = comdMalloc(ll->columnNumNeighbours[iColumn]*sizeof(MPI_Status));
            // Different request for each neighbour
            ll->recvRequests[iColumn] = comdMalloc(ll->columnNumNeighbours[iColumn]*sizeof(MPI_Request));

            // Different send and receive buffer for each neighbour
            ll->sendBufs[iColumn] = comdMalloc(ll->columnNumNeighbours[iColumn]*sizeof(char*));
            ll->recvBufs[iColumn] = comdMalloc(ll->columnNumNeighbours[iColumn]*sizeof(char*));
            for (int iColumnNeighbour=0; iColumnNeighbour<ll->columnNumNeighbours[iColumn]; ++iColumnNeighbour)
            {
                // Buffer capacity is preset
                ll->sendBufs[iColumn][iColumnNeighbour] = comdMalloc(haloExchange->bufCapacity);
                ll->recvBufs[iColumn][iColumnNeighbour] = comdMalloc(haloExchange->bufCapacity);
            }
        }
    }
...
#pragma oss task firstprivate(iColumn) \
            in(ll->secondUpdateSentinel[iColumn],ll->secondMallocSentinel[iColumn]) \
            out(ll->loadSentinel[iColumn]) 
            {
                // LOADS
                for (int iColumnNeighbour=0; iColumnNeighbour<ll->columnNumNeighbours[iColumn]; ++iColumnNeighbour)
                {
                    ...
                    if (ll->columnCommNeighbours[iColumn][iColumnNeighbour][2] == 0)
                    {
                        ll->nSends[iColumn][iColumnNeighbour] = haloExchange->loadColumnBuffer(haloExchange->parms,
                                data,
                                x,
                                y,
                                ll->columnCommNeighbours[iColumn][iColumnNeighbour][1],
                                ll->sendBufs[iColumn][iColumnNeighbour]);
                    }
                    else
                    {
                        ll->nSends[iColumn][iColumnNeighbour] = haloExchange->loadColumnEndBuffer(haloExchange->parms,
                                data,
                                x,
                                y,
                                ll->columnCommNeighbours[iColumn][iColumnNeighbour][1],
                                ll->columnCommNeighbours[iColumn][iColumnNeighbour][2],
                                ll->sendBufs[iColumn][iColumnNeighbour]);
                    }
                }
            }
    ...
    for (int x=-1;x<ll->gridSize[0]+1;++x)
    {
        for (int y=-1;y<ll->gridSize[1]+1;++y)
        {
            iColumn = y+1+(x+1)*(ll->gridSize[1]+2);
#pragma oss task firstprivate(iColumn) \
            in(ll->loadSentinel[iColumn]) \
            out(ll->sendSentinel[iColumn])
            {
                for (int iColumnNeighbour=0;iColumnNeighbour<ll->columnNumNeighbours[iColumn];++iColumnNeighbour)
                {
                    int xoffset = ll->gridSize[0] + 1;
                    int yoffset = ll->gridSize[1] + 1;
                    int xmod = ll->gridSize[0]*2;
                    int ymod = ll->gridSize[1]*2;
                    int newx = (x+xoffset)%(xmod) - 1;
                    int newy = (y+yoffset)%(ymod) - 1;

                    // Set newx and newy based on which neighbour is being sent to 
                    switch(ll->columnCommNeighbours[iColumn][iColumnNeighbour][1])
                    {
                        case HALO_X_MINUS:
                            newy = y;
                            break;
                        ...
                        default:
                            printf("Unrecognised neighbour face\n");
                    }

                    int tag = newy+1+(newx+1)*(ll->gridSize[1]+2);

                    ll->sendRequests[iColumn][iColumnNeighbour] = isendParallel(ll->sendBufs[iColumn][iColumnNeighbour],
                            ll->nSends[iColumn][iColumnNeighbour],
                            ll->columnCommNeighbours[iColumn][iColumnNeighbour][0],
                            tag);

                    TAMPI_Iwait(&ll->sendRequests[iColumn][iColumnNeighbour], MPI_STATUS_IGNORE);
                }
            }
        }
    }
    ...
    {
#pragma oss task firstprivate(iColumn) \
        in(ll->unloadSentinel[iColumn],ll->secondMallocSentinel[iColumn]) \
        out(ll->recvSentinel[iColumn])
        {
            for (int iColumnNeighbour=0; iColumnNeighbour<ll->columnNumNeighbours[iColumn]; ++iColumnNeighbour)
            {
                MPI_Irecv(ll->recvBufs[iColumn][iColumnNeighbour], haloExchange->bufCapacity, MPI_BYTE, ll->columnCommNeighbours[iColumn][iColumnNeighbour][0], iColumn, MPI_COMM_WORLD, &ll->recvRequests[iColumn][iColumnNeighbour]);
                TAMPI_Iwait(&ll->recvRequests[iColumn][iColumnNeighbour], &ll->recvStatus[iColumn][iColumnNeighbour]);
            }
        }
    }
...
#pragma oss task firstprivate(iColumn) \
            in(ll->recvSentinel[iColumn],ll->sendSentinel[iColumn]) \
            out(ll->unloadSentinel[iColumn])
            {
                for (int iColumnNeighbour=0; iColumnNeighbour<ll->columnNumNeighbours[iColumn]; ++iColumnNeighbour)
                {
                    MPI_Get_count(&ll->recvStatus[iColumn][iColumnNeighbour], MPI_BYTE, &ll->nRecvs[iColumn][iColumnNeighbour]);
                    haloExchange->unloadBoxBuffer(haloExchange->parms,
                            data,
                            0,
                            ll->nRecvs[iColumn][iColumnNeighbour],
                            ll->recvBufs[iColumn][iColumnNeighbour]);
                }
                for (int iCell=-1; iCell<ll->gridSize[2]+1; ++iCell)
                {
                    int iBox = getBoxFromTuple(ll,x,y,iCell);
                    int nAtoms = ll->nAtoms[iBox];
                    AtomMsg tmp[nAtoms];
                    int begin = iBox*MAXATOMS;

                    for (int iOff=begin,ii=0; ii<nAtoms; ii++,iOff++)
                    {
                        tmp[ii].gid  = s->atoms->gid[iOff];
                        tmp[ii].type = s->atoms->iSpecies[iOff];
                        tmp[ii].rx =   s->atoms->r[iOff][0];
                        tmp[ii].ry =   s->atoms->r[iOff][1];
                        tmp[ii].rz =   s->atoms->r[iOff][2];
                        tmp[ii].px =   s->atoms->p[iOff][0];
                        tmp[ii].py =   s->atoms->p[iOff][1];
                        tmp[ii].pz =   s->atoms->p[iOff][2];
                    }
                    ...
                    for (int iOff=begin,ii=0; ii<nAtoms; ii++,iOff++)
                    {
                        s->atoms->gid[iOff]   = tmp[ii].gid;
                        s->atoms->iSpecies[iOff] = tmp[ii].type;
                        s->atoms->r[iOff][0]  = tmp[ii].rx;
                        s->atoms->r[iOff][1]  = tmp[ii].ry;
                        s->atoms->r[iOff][2]  = tmp[ii].rz;
                        s->atoms->p[iOff][0]  = tmp[ii].px;
                        s->atoms->p[iOff][1]  = tmp[ii].py;
                        s->atoms->p[iOff][2]  = tmp[ii].pz;
                    }
                }
            }
        }
    ...
    {
#pragma oss task firstprivate(iColumn) \
        in(ll->unloadSentinel[iColumn]) \
        out(ll->firstFreeSentinel[iColumn])
        {
            for (int iColumnNeighbour=0; iColumnNeighbour<ll->columnNumNeighbours[iColumn]; ++iColumnNeighbour)
            {
                comdFree(ll->recvBufs[iColumn][iColumnNeighbour]);
                comdFree(ll->sendBufs[iColumn][iColumnNeighbour]);
            }
            comdFree(ll->sendRequests[iColumn]);
            comdFree(ll->sendBufs[iColumn]);
            comdFree(ll->recvBufs[iColumn]);
            comdFree(ll->recvRequests[iColumn]);
            comdFree(ll->nRecvs[iColumn]);
            comdFree(ll->recvStatus[iColumn]);
        }
    }

#pragma oss task in({ll->firstFreeSentinel[index],index=0;ll->numColumns}) \
    out(ll->secondFreeSentinel[0])
    {
        comdFree(ll->sendBufs);
        comdFree(ll->recvBufs);
        comdFree(ll->sendRequests);
        comdFree(ll->recvRequests);
        comdFree(ll->nSends);
        comdFree(ll->nRecvs);
        comdFree(ll->recvStatus);
    }
}
