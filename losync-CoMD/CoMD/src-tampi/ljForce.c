...
typedef struct LjPotentialSt
{
    ...
    int  (*force)(SimFlat* s, int withEnergy); //!< function pointer to force routine
...
static int ljForce(SimFlat* s, int withEnergy);
...
int ljForce(SimFlat* s, int withEnergy)
{
    if(withEnergy)
    {
...
#pragma oss task firstprivate(iColumn) \
                in({ll->unloadSentinel[ll->internalNeighbours[iColumn][index]], index=0;ll->numInternalNeighbours[iColumn]}) \
                out(ll->zeroSentinel[iColumn])
                {
        ...
        for (int x=-1;x<ll->gridSize[0]+1;++x)
        {
            for (int y=-1;y<ll->gridSize[1]+1;++y)
            {
                int iColumn = y+1+(x+1)*(ll->gridSize[1]+2);
#pragma oss task firstprivate(iColumn) \
                in(ll->zeroSentinel[iColumn]) \
                out(ll->forceSentinel[iColumn])
                {
