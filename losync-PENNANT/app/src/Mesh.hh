...
public:
    ...
    int** sumOnProctaskindices;    // simplifies task dependencies for local sum on proc
    int* sumOnProctasksizes;       // size of each sumOnProctaskindices[pch] entry

    double* prxvar_double;                  // buffers for sumAcrossProcs
    double2* prxvar_double2;
    double* slvvar_gather_double;           // buffers for parallelGather
    double2* slvvar_gather_double2;
    double* slvvar_scatter_double;          // buffers for parallelScatter
    double2* slvvar_scatter_double2;
    double* giveprxvar(double* array);      // helpers to select correct buffer type
    double2* giveprxvar(double2* arrayt);
    double* giveGatherBuf(double* array);
    double2* giveGatherBuf(double2* arrayt);
    double* giveScatterBuf(double* array);
    double2* giveScatterBuf(double2* array);
