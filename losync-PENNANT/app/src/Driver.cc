...
void Driver::run() {
    ...
    while (cycle < cstop && time < tstop) {
        ...
        // get timestep
        // TODO: Global synchronisation point. Cannot execute until all local
        // dt calculations across all chunks have completed.
        #pragma oss task label("Calculate global Dt") \
                         in(hydro->dtrec[0:mesh->numz-1]) \
                         out(hydro->dt[cycle-1]) \
                         inout(dt) \
                         firstprivate(cycle)
        {
            calcGlobalDt();
            hydro->dt[cycle-1] = dt; // -1 as cycle counts from 1 but we index from 0
        }

        // begin hydro cycle
        hydro->doCycle(cycle);

        // TODO: Ignoring this in tasked version. Exclusively using cycle count as stopping criteria
        // TODO URGENT: CAN'T IGNORE THIS! It's used in calcGlobalDt()
        time += dt;
    ...
    // Main thread waits for all tasks in main event loop to complete
    #pragma oss taskwait
...
void Driver::calcGlobalDt() {
    ...
    dtlast = dt; // TODO: do we need to turn dtlast into an array? Don't think we can accidentally read a dtlast from the wrong cycle
    msgdtlast = msgdt;
